...
const Services = () => {
  return (
      <Box padding={{ base: '21px 24px', md: '82px 72px' }} color={'color.HeadingColor'}>
          <Text fontSize={{base: '2xl', md: '3xl'}} fontWeight={'700'}>Photography Services</Text>
          <Stack display={'flex'} flexDirection={{ base: 'column', md: 'row' }} justifyContent={'center'} alignItems={'center'}>
            {
                photoServices.map((service, index) => (
                    ...
                    >
                        <Stack>
                            <Image src={service.imageUrl} alt={service.title} borderTopRadius={'12px'} w={'280px'} height={'350px'} objectFit={'cover'}/>
                        </Stack>
